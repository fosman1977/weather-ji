...
interface Stadium {
  ...
  covered: number; // percentage of seats covered
}
...
export default function PitchCoverPage() {
  ...
  const fetchWeather = async () => {
    ...
    try {
      ...
      // Calculate rain risk (weighted average of next 6 hours)
      const criticalHours = next12Hours.slice(0, 6);
      const rainRisk = Math.round(
        criticalHours.reduce((sum, h, i) => {
          const weight = 1 - (i * 0.1); // Closer hours weighted more
  ...
  const calculatePremium = () => {
    ...
    const baseRate = 0.08; // 8% of ticket value
    ...
    const coveredDiscount = 1 - (selectedStadium.covered / 200); // Max 50% discount
    ...
    return Math.min(premium, ticketValue * 0.95); // Cap at 95% of ticket value
  };
  ...
  const simulateMatch = () => {
    ...
    // Realistic simulation based on actual rain probability
              ...
              {matchResult === 'rained' ? (
                ...
                <>
                  ...
                  <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-6 mb-6">
                    ...
                    <div className="text-xs text-amber-600 mt
